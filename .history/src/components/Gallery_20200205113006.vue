...
<v-container>
  ...
  <v-row>
      <v-col
        v-for="(image, index) in this.galleryHead"
        :key="index"
        cols=4
...
export default {
  ...
  computed: {
    ...
    galleryHead() {
      ...
      for (i=0, i++ , i<this.galleries.length) {
          console.log( this.galleries[i][0]);
      }
      const gl = this.galleries.map(item => this.galleryHeads.push(item[0]));
      console.log('gl: ', gl);
      this.galleryHeads = gl.map((item) => item[0]);
      // this.$set(this.galleryHeads, 0, this.galleries.map((item) => item[0]));
      // this.galleryHeads = this.galleries.map((item) => item[0]);
