...
export default {
  ...
  computed: {
    fetchData() {
      galleries = this.$t('galleries');
      return galleries;
    },
    galleryHead() {

      console.log('this.galleries: ', this.galleries);
      // const gl = Array.from(this.galleries);
      const gl = galleries.filter((item) => item.index===0);
      console.log('gl: ', gl);
      // const this.galleryHeads = this.galleries.map((item) => {
        // console.log(item);
      // })
    },
  },
